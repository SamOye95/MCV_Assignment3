		...
		Camera* camera)
{
	...
	{
		Mat grayImage, g_background, diff;
		// Convert the frame to grayScale
		cvtColor(Image, grayImage, CV_BGR2GRAY);
		// Merge the background HSV channels into one image.
		merge(camera->getBgHsvChannels(), background);

		// Convert the background to grayscale
		cvtColor(background, background, CV_HSV2BGR);
		cvtColor(background, g_background, CV_BGR2GRAY);

		// Calculate the difference between background and foreground
		absdiff(grayImage, g_background, diff);

		// Find threshold using Otsu's method and apply it
		double thres = threshold(diff, background, 0, 255, THRESH_BINARY_INV + THRESH_OTSU);
		threshold(diff, foreground, thres, 255, THRESH_BINARY_INV);
